...
class ArenaApp(QMainWindow):
    ...
    def actualizar_posiciones(self):
        """Actualizar posiciones de todos los elementos UI de manera organizada"""
        # Verificar que todos los widgets existen antes de attemptar moverlos
        widgets_configuracion = [
            'btn_nivel_down', 'nivel_label', 'btn_nivel_up',
            'btn_apuesta_down', 'apuesta_label', 'btn_apuesta_up'
        ]
        
        for widget_name in widgets_configuracion:
            if not hasattr(self, widget_name) or getattr(self, widget_name) is None:
                print(f"Advertencia: Widget {widget_name} no está inicializado")
                return
        
        # Posiciones relativas basadas en el tamaño de la ventana
        positions = {
        ...
        for element, (rel_x, rel_y) in positions.items():
            ...
            if widget is not None:  # Verificación adicional
                x = int(self.width() * rel_x - (40 if 'btn' in element else 30))
                y = int(self.height() * rel_y - (40 if 'btn' in element else 20))
                widget.move(x, y)
            else:
                print(f"Advertencia: Widget {element} es None")
        ...
        if hasattr(self, 'reward_log') and self.reward_log is not None:
            self.reward_log.move(
                ...
                int(self.height() * 0.53 + self.event_log.height()/2 + 20)
            )
        ...
        if hasattr(self, 'botones') and self.botones:
            for i, btn in enumerate(self.botones):
                if btn is not None:  # Verificar que el botón no sea None
                    x = int(self.width() * btn_positions[i][0] - 40)
                    y = int(self.height() * btn_positions[i][1] - 40)
                    btn.move(x, y)
    ...
    def configurar_paneles_configuracion(self, parent):
        ...
        if self.nivel_label:
            ...
            self.nivel_label.setStyleSheet("""
                QLabel {
            ...
            """)
            self.nivel_label.setFixedSize(60, 40)
        else:
        ...
        if self.apuesta_label:
            ...
            self.apuesta_label.setStyleSheet("""
                QLabel {
                    background-color: transparent;
                    color: black;
                    border: none;
            ...
            """)
            self.apuesta_label.setFixedSize(80, 40)
        else:
    ...
    def configurar_log(self, parent):
        ...
        """)
        self.event_log.setFixedSize(1000, 300)
    ...
    def _configurar_formatos_texto(self):
        ...
        formatos = {
            "titulo": ("#000000", 16, True, False),
            "recompensa": ("#006400", 13, False, False),
            "enemigo": ("#8B0000", 16, True, False),
            "lista": ("#000000", 12, False, False),
            "ronda": ("#000000", 14, True, False),
            "heroico": ("#006400", 13, True, False),
            "deshonroso": ("#8B0000", 13, True, False),
            "publico": ("#000000", 13, False, True),
            "efecto": ("#000000", 13, False, False),
            "critical": ("#8B0000", 13, True, False),
            "blink": ("#000000", 13, False, False),
            "apuesta": ("#006400", 13, True, False),
            "center": ("#000000", 14, False, False),
            "speaker": ("#8B4513", 14, True, True)  # NUEVO FORMATO PARA EL SPEAKER
        }
    ...
    def configurar_log_recompensas(self, parent):
        ...
        """)
        self.reward_log.setFixedSize(450, 250)
